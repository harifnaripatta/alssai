...
class DonationBanner {
  ...
  static setDonationCookie(daysCookieWillExpire) {
    ...
    Cookies.set(
      "donation",
      "x",
      $.extend(cookie, { domain: ".openlibrary.org" })
    );
  }
  ...
  setupDonationForm() {
    ...
    if (isPsaBanner) {
      ...
      ctaButton.addEventListener("click", (event) => {
        ...
        console?.log(
          "donationInfoChanged - IN minimal banner check - post cookie",
          { url }
        );
    ...
    editDonationForm.addEventListener("donationInfoChanged", (event) => {
      ...
      const isMinimalBanner =
        this.bannerElement.classList.contains("formdesign-minimal");
    ...
    continueButton.addEventListener("click", () => {
      ...
      let customSelectedTextColor = bannerDataSet.selectedtextcolor
        ? `&selectedTextColor=${encodeURIComponent(
            bannerDataSet.selectedtextcolor
          )}`
        : "";
      let customSelectedFillColor = bannerDataSet.selectedfillcolor
        ? `&selectedFillColor=${encodeURIComponent(
            bannerDataSet.selectedfillcolor
          )}`
        : "";
      /* allow for override to custom selected option colors for donate bage */
      if (bannerDataSet.donatepageselectedfillcolor) {
        customSelectedFillColor = `&selectedFillColor=${encodeURIComponent(
          bannerDataSet.donatepageselectedfillcolor
        )}`;
      }
      if (bannerDataSet.donatepageselectedtextcolor) {
        customSelectedTextColor = `&selectedTextColor=${encodeURIComponent(
          bannerDataSet.donatepageselectedtextcolor
        )}`;
      }
  ...
  logEvent(eventAction, sendAsSampledEvent = false) {
    ...
    if (
      sendAsSampledEvent ||
      (eventAction === "Viewed" && (platform === "ia" || platform === "wb"))
    ) {
